...
for(int q=0;q<ntempquery.length;q++)
           				{
							...
							try{
								...
								String sql9="select * from view where contentname='"+ntempquery[q]+"' and uid='"+uid+"' and concept='concept' ";
								rs9=st.executeQuery(sql9);
								while(rs9.next()){
									try {
								
											PreparedStatement ps=con.prepareStatement("update tempcontent set count=? where contentname='"+ntempquery[q]+"' ");
 ...
 <%
	...
	try{
			...
			while(rs7.next()){	%>			
				...
				<p><a href="#"><%=rs7.getString("contentname")%></a><br /> Title:--<a href="contentcount.jsp?id=<%=rs7.getInt("id")%>,<%=rs7.getString("contentname")%>"><%=rs7.getString("title")%></a></p>
				<p>Views:--<a><%=rs7.getInt("count")%></a></p>
